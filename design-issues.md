...
## 13. No labeled `break` or `continue` for nested loops

**Where:** §6.9, §6.10

`break` and `continue` affect only the immediately enclosing loop. Breaking out of an outer
loop from inside an inner loop requires a boolean flag variable, which costs at least one
register and 2–3 extra instructions per nested-break site. Hand-written ic10 simply emits
`j outer_end` — a single instruction.

**Required fix:** Add labeled break and continue, following Rust syntax: `'label: loop { …
break 'label; … }`. `break 'label` compiles to a single `j` to the post-loop label; no
flag variable is needed.

---

## 15. The `for` loop supports only ascending, step-1 ranges

**Where:** §6.8

The `for i in a..b` construct always iterates in ascending order with a step of 1. Common
ic10 patterns that require a different traversal include:

- **Counting down** to zero: the back-edge test `bnez r_i loop` is cheaper than
  `blt r_i r_upper loop` because it tests against zero and can use the zero-immediate
  branch form, saving a register for the upper bound.
- **Step sizes other than 1**: iterating over every other slot, or jumping by
  a device count stride.
- **Inclusive upper bounds**: `0..=5` is more natural than `0..6` when the bound is a
  compile-time constant and avoids a `+ 1` at the call site.

Without these forms, programs are forced into `while` loops, which the compiler cannot
identify as counted loops and therefore cannot apply loop-specific optimizations to.

**Required fix:** At minimum, specify that when both bounds of a `for` loop are constants and
the count is fixed, the compiler should generate a bottom-tested loop (see issue 16). Consider
adding a reverse iteration form `for i in (a..b).rev()` and an inclusive upper bound `a..=b`.
A step form `for i in (a..b).step(n)` would cover the remaining cases.

---

## 16. `for` and `while` loops use a top-tested pattern that costs an extra instruction per iteration

**Where:** §6.7, §6.8

The compilation templates in §6.7 and §6.8 place the exit test at the **top** of the loop,
meaning the back-edge is an unconditional `j loop_start`, and the exit test + branch are at
the top before the body. This costs 2 control-flow instructions per iteration (the conditional
branch at the top and the unconditional `j` at the bottom).

A bottom-tested loop places the body first and the conditional branch at the bottom as the
back-edge:

```ic10
<init>
bge r_i r_upper for_end    # skip empty range
for_start:
<body>
add r_i r_i 1
blt r_i r_upper for_start  # 1 instruction: back-edge test
for_end:
```

This costs 1 control-flow instruction per iteration — exactly what hand-written ic10 uses.
For a loop that executes N times, the top-tested form spends N extra unconditional jumps
compared to the bottom-tested form. In a loop over 6 devices, that is 6 wasted lines.

**Required fix:** The spec should explicitly require (or at least permit and recommend) that
compilers generate bottom-tested loops for `for` and `while` constructs. The compilation
templates in §6.7 and §6.8, being non-normative, should be updated to show the bottom-tested
form to guide implementors toward the correct approach.
